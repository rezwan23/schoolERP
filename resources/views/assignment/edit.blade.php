...
@section('content')
    ...
    <div class="row">
        <div class="col-md-10">
            <div class="tile">
                <h3 class="tile-title">Edit Assignment</h3>
                ...
                <form action="{{route('assignment.update', $assignment)}}" method="post">
                    ...
                    <div class="tile-body">
                        <div class="form-group">
                            ...
                            <input class="form-control" type="text" value="{{$assignment->title}}" required name="title" placeholder="Enter Title">
                        </div>
                        <div class="form-group">
                            <label class="control-label">Class</label>
                            <select name="class_id" id="" class="form-control">
                                @foreach($classes as $class)
                                    <option @if($assignment->class_id == $class->id) selected @endif value="{{$class->id}}">{{$class->name}}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group">
                            <label class="control-label">Assignment</label>
                            <textarea id="content" name="assignment" cols="30" required rows="10">{{$assignment->content}}</textarea>
                        </div>
                        <div class="form-group">
                            <label class="control-label">Submit Date</label>
                            <input class="form-control" id="demoDate" value="{{$assignment->submit_date}}" type="text" required name="submit_date" placeholder="Enter Date">
                        </div>
                    </div>
